...
class MusicBot(discord.Client):
    ...
    async def cmd_help(self, command=None):
        ...
        if command:
            ...
            if cmd:
                ...
                return Response("não é um comando", delete_after=10)
    ...
    async def cmd_blacklist(self, message, user_mentions, option, something):
        ...
        Adicionar ou remover membros Da BlackList.
        membros que estão na Blacklist não podem usar comandos do chat!.
        """

        if not user_mentions:
            raise exceptions.CommandError("nenhúm membro listado.", expire_in=20)

        if option not in ['+', '-', 'add', 'remove']:
            raise exceptions.CommandError(
                'Opção Inválida "%s" specified, use +, -, add, or remove' % option, expire_in=20
            )

        for user in user_mentions.copy():
            if user.id == self.config.owner_id:
                print("[Commands:Blacklist] O Dono não pode ser Colocado Na BlackList.")
        ...
        if option in ['+', 'add']:
            ...
            return Response(
                '%s Membros foram adicionados à BlackList' % (len(self.blacklist) - old_len),
                reply=True, delete_after=10
            )

        else:
            if self.blacklist.isdisjoint(user.id for user in user_mentions):
                return Response('nenhúm desses membros está na BlackList.', reply=True, delete_after=10)
                ...
                return Response(
                    '%s Membros foram Removidos da BlackList' % (old_len - len(self.blacklist)),
                    reply=True, delete_after=10
                )
    ...
    async def cmd_id(self, author, user_mentions):
        ...
        Mostra o id da pessoa ou de alguém.
        """
        if not user_mentions:
            return Response('O seu Id é `%s`' % author.id, reply=True, delete_after=35)
        else:
            usr = user_mentions[0]
            return Response("%s id é `%s`" % (usr.name, usr.id), reply=True, delete_after=35)

    @owner_only
    async def cmd_invite(self, message, server_link=None):
        """
        Usage:
            {command_prefix}Convite invite_link
        ...
        if self.user.bot:
            ...
            return Response(
                "O bot não pode utilizar links para entrar em outros servidores,para me adicionar vá em: \n{}".format(url),
        ...
        try:
            ...
            raise exceptions.CommandError('URL inválido:\n{}\n'.format(server_link), expire_in=30)
    ...
    async def cmd_play(self, player, channel, author, permissions, leftover_args, song_url):
        ...
        if permissions.max_songs and player.playlist.count_for_user(author) >= permissions.max_songs:
            raise exceptions.PermissionsError(
                "Você alcançou o seu limite de Músicas em espera! (%s)" % permissions.max_songs, expire_in=30
            )
        ...
        if not info:
            raise exceptions.CommandError("Esse vídeo não pode ser Tocado!.", expire_in=30)
        ...
        if 'entries' in info:
            ...
            if not permissions.allow_playlists and ':search' in info['extractor'] and len(info['entries']) > 1:
                raise exceptions.PermissionsError("Você não tem permissão para utilizar Playlists", expire_in=30)
            ...
            if permissions.max_playlist_length and num_songs > permissions.max_playlist_length:
                raise exceptions.PermissionsError(
                    "Playlist tem muitas músicas (%s > %s)" % (num_songs, permissions.max_playlist_length),
                    expire_in=30
                )
    ...
    async def _cmd_play_playlist_async(self, player, channel, author, permissions, playlist_url, extractor_type):
        ...
        if not info:
            raise exceptions.CommandError("Essa playlist não pode ser tocada.")

        num_songs = sum(1 for _ in info['entries'])
        t0 = time.time()

        busymsg = await self.safe_send_message(
            channel, "Processando %s músicas..." % num_songs)  # TODO: From playlist_title
        await self.send_typing(channel)
    ...
    async def cmd_search(self, player, channel, author, permissions, leftover_args):
        ...
        if permissions.max_songs and player.playlist.count_for_user(author) > permissions.max_songs:
            raise exceptions.PermissionsError(
                "você alcançou o limite máximo de Música em espera! (%s)" % permissions.max_songs,
                expire_in=30
            )
        ...
        try:
            ...
            raise exceptions.CommandError("Por favor Utilize o search corretamente.", expire_in=30)

        service = 'youtube'
        items_requested = 3
        max_items = 1000  # this can be whatever, but since ytdl uses about 1000, a small number might be better
        services = {
            'youtube': 'ytsearch',
            'soundcloud': 'scsearch',
            'yahoo': 'yvsearch',
        ...
        if leftover_args[0].isdigit():
            ...
            if items_requested > max_items:
                raise exceptions.CommandError("Você não pode procurar por mais de %s videos" % max_items)
        ...
        search_msg = await self.send_message(channel, "Buscando por música...")
        await self.send_typing(channel)
        ...
        if not info:
            return Response("Nenhum vídeo encontrado.", delete_after=30)

        def check(m):
            return (
                m.content.lower()[0] in 'simprox' or
                # hardcoded function name weeee
                m.content.lower().startswith('{}{}'.format(self.config.command_prefix, 'search')) or
                m.content.lower().startswith('sair'))

        for e in info['entries']:
            result_message = await self.safe_send_message(channel, "Resultados %s/%s: %s" % (
                info['entries'].index(e) + 1, len(info['entries']), e['webpage_url']))

            confirm_message = await self.safe_send_message(channel, "Isso está certo?? Escreva `sim`, `prox` or `sair`")
            response_message = await self.wait_for_message(30, author=author, channel=channel, check=check)

            if not response_message:
                await self.safe_delete_message(result_message)
                await self.safe_delete_message(confirm_message)
                return Response("Ok,deixa para lá.", delete_after=30)

            # They started a new search query so lets clean up and bugger off
            elif response_message.content.startswith(self.config.command_prefix) or \
                    response_message.content.lower().startswith('sair'):

                await self.safe_delete_message(result_message)
                await self.safe_delete_message(confirm_message)
                return

            if response_message.content.lower().startswith('sim'):
                await self.safe_delete_message(result_message)
                await self.safe_delete_message(confirm_message)
                ...
                return Response("Tudo Bem, isso está por vir!", delete_after=30)
            else:
                await self.safe_delete_message(result_message)
                await self.safe_delete_message(confirm_message)
                await self.safe_delete_message(response_message)

        return Response("A está bom :frowning:", delete_after=30)
    ...
    async def cmd_summon(self, channel, author, voice_channel):
        ...
        if not author.voice_channel:
            raise exceptions.CommandError('Você não está em um canal de voz!!')
        ...
        if not chperms.connect:
            self.safe_print("não posso entrar \"%s\", não tenho permissão." % author.voice_channel.name)
            return Response(
                "```não posso entrar \"%s\", não tenho permissão.```" % author.voice_channel.name,
                delete_after=25
            )

        elif not chperms.speak:
            self.safe_print("não consigo entrar \"%s\", não tenho permissão para falar." % author.voice_channel.name)
            return Response(
                "```não consigo entrar \"%s\", não tenho permissão para falar.```" % author.voice_channel.name,
                delete_after=25
            )
    ...
    async def cmd_pause(self, player):
        ...
        if player.is_playing:
            ...
            raise exceptions.CommandError('O bot não está tocando nada.', expire_in=30)
    ...
    async def cmd_resume(self, player):
        ...
        if player.is_paused:
            ...
            raise exceptions.CommandError('O bot não está pausado', expire_in=30)

    async def cmd_S2(self, channel, player):
        """
        Usage:
    ...
    async def cmd_skip(self, player, channel, author, message, permissions, voice_channel):
        ...
        if player.is_stopped:
            raise exceptions.CommandError("o Bot não pode dar skip,pois o mesmo não está tocando nada!", expire_in=20)

        if not player.current_entry:
            if player.playlist.peek():
                if player.playlist.peek()._is_downloading:
                    # print(player.playlist.peek()._waiting_futures[0].__dict__)
                    return Response("A proxima música (%s) está em download, por favor espere." % player.playlist.peek().title)

                elif player.playlist.peek().is_downloaded:
                    print("A proxima música será tocada daqui à pouco.  espere!.")
                else:
    ...
    async def cmd_volume(self, message, player, new_volume=None):
        ...
        if not new_volume:
            return Response('Volume atual: `%s%%`' % int(player.volume * 100), reply=True, delete_after=20)
        ...
        try:
            ...
            raise exceptions.CommandError('{} não é um número válido'.format(new_volume), expire_in=20)
        ...
        if 0 < new_volume <= 100:
            ...
            return Response('updated volume de %d para %d' % (old_volume, new_volume), reply=True, delete_after=20)
    ...
    async def cmd_queue(self, channel, player):
        ...
        lines = []
        unlisted = 0
        andmoretext = '* ... e %s more*' % ('x' * len(player.playlist.entries))
        ...
        if player.current_entry:
            ...
            if player.current_entry.meta.get('channel', False) and player.current_entry.meta.get('author', False):
                lines.append("Agora Tocando: **%s** Adicionado por **%s** %s\n" % (
                    player.current_entry.title, player.current_entry.meta['author'].name, prog_str))
            else:
                lines.append("Agora tocando: **%s** %s\n" % (player.current_entry.title, prog_str))
        ...
        if not lines:
            lines.append(
                'Não tem música na lista de espera! Queue something with {}play.'.format(self.config.command_prefix))
